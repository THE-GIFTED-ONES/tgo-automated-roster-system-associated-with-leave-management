...
exports.createNewUser = catchAsync(async (req, res, next) => {
  const { email, empName, empID } = req.body;
  ...
  const resetToken = newUser.createInitialPasswordResetToken();

  const resetURL = `${req.protocol}://${req.get(
    'host'
  )}/api/v1/users/firstResetPassword/${resetToken}`;

  SendWelcomeEmail(email, empName, resetURL);
...
exports.firstPasswordReset = catchAsync(async (req, res, next) => {
  // 1) Get user based on the token
  const hashedToken = crypto
    .createHash('sha256')
    .update(req.params.token)
  ...
  const user = await User.findOne({ intialpasswordResetToken: hashedToken });

  // 2) If there is user, set the new password
  user.password = req.body.password;
  user.passwordConfirm = req.body.passwordConfirm;
  user.firstlogin = false;
  user.intialpasswordResetToken = undefined;
